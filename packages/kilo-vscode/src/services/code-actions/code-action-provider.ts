...
export class KiloCodeActionProvider implements vscode.CodeActionProvider {
  ...
  provideCodeActions(
    ...
    add.command = { command: "relay.new.addToContext", title: "Add to Kilo Code" }
    actions.push(add)
    ...
    if (hasDiagnostics) {
      ...
      fix.command = { command: "relay.new.fixCode", title: "Fix with Kilo Code" }
    ...
    if (!hasDiagnostics) {
      ...
      explain.command = { command: "relay.new.explainCode", title: "Explain with Kilo Code" }
      actions.push(explain)

      const improve = new vscode.CodeAction("Improve with Kilo Code", vscode.CodeActionKind.RefactorRewrite)
      improve.command = { command: "relay.new.improveCode", title: "Improve with Kilo Code" }
